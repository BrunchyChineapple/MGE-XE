...
void StlVector::sort_by_state() {
	if (m_vector.size() > 0) {
		std::stable_sort(m_vector.begin(), m_vector.end(), [](const RenderMesh* lhs, const RenderMesh* rhs) {
...
#ifdef MGE_RTX
void StlVector::retain_with_ttl() {
	std::unordered_set<const RenderMesh*> currentSet(m_vector.begin(), m_vector.end());

	// Update TTL for retained meshes
	auto it = m_retained.begin();
	while (it != m_retained.end()) {
		if (currentSet.count(it->first)) {
			...
			if (it->second <= 0) {
				...
				m_vector.push_back(it->first);
				++it;
			}
		}
	}

	// Add newly departed meshes to retention
	for (const auto* mesh : m_previousVisible) {
		if (!currentSet.count(mesh) && !m_retained.count(mesh)) {
			m_retained[mesh] = RTX_MESH_TTL;
			m_vector.push_back(mesh);
		}
	}

	m_previousVisible.assign(currentSet.begin(), currentSet.end());
}
#endif
...
void IpcClientVector::restart() {
	// After clear() + server write, m_index may already be 0 from a previous
	// restart that ran while the vector was empty. set_index(0) short-circuits
	// when m_index == 0, leaving the window unmapped. Force a remap by moving
	// to index 1 first (only when the vector has data).
	if (m_view.size() > 1) {
		m_view.set_index(1);
	}
	m_view.set_index(0);
