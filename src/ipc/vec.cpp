...
namespace IPC {
		...
		m_buffer(nullptr)
	{
		...
		// Reserve one window's worth of bytes per window (m_maxSize is now an exact multiple of
		// m_windowSize, so this division is exact). The previous formula m_maxSize * m_windowBytes
		// over-counted by a factor of m_windowSize -- it multiplied the TOTAL element count by the
		// per-WINDOW byte size -- which overflowed the uint32 m_reservedBytes. For the composite
		// channels (e.g. maxSize 65536 * windowBytes 65536 == 2^32) it wrapped to 0, so the buffer
		// MapViewOfFile was asked for length 0 and returned ACCESS_DENIED. The client (view.cpp)
		// only ever maps one window at a time at offset currentWindow*windowBytes+headerBytes, so
		// numWindows*windowBytes is the exact span the reservation needs to cover.
		m_reservedBytes = static_cast<std::uint32_t>(
			(static_cast<std::uint64_t>(m_maxSize) / m_windowSize) * m_windowBytes);
