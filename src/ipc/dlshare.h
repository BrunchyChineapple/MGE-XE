...
public:
    ...
    // Architecture B: the full per-cell composite pool held in the 64-bit server's
    // address space, plus its CellId/chunk indices (task 6.1 Format_Loader populates it,
    // task 6.2 Composite_Streamer reads it). compositePool.active is false for every
    // Old/absent/malformed load, which keeps the streamer inert and the renderer on the
    ...
    // inactive single-atlas-only state and returns false WITHOUT throwing (it must never
    // raise across the IPC boundary -- Requirements 4.6, 6.3). The single load-time log
    // line is emitted here.
    static bool loadCompositeSet();
    static bool setCurrentWorldSpace(const char* name);    static void getVisibleMeshesCoarse(IPC::Vec<RenderMesh>& output, const ViewFrustum& viewFrustum, VisibleSetSort sort, DWORD setFlags);
